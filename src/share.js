...
const services = {
  ...
  pinterest: {
    url:
      "https://www.pinterest.com/pin/create/button/?url=%url%&media=%image_url%&description=%description%&title=%title%",
    name: "Pinterest",
...
export function getShareUrl(service, item) {
  ...
  return Object.keys(tags).reduce(
    (url, tag) => url.replace(`%${tag}%`, encodeURIComponent(tags[tag])),
    service.url
  );
}
